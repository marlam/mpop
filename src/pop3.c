        ...
        char **errstr)
{
    ...
    e = net_open_socket(socketname, proxy_hostname, proxy_port,
...
int pop3_capa(pop3_session_t *session, char **errstr)
{
    ...
    for (count = 0; count < POP3_MAX_CAPAREPLY_LINES; count++)
    {
        ...
        else
        {
            ...
            {
                const char *authstring = session->buffer + 5;
                while (session->buffer[i])
                {
                    session->buffer[i] =
                        toupper((unsigned char)session->buffer[i]);
                ...
                if (token_in_string(authstring, "SCRAM-SHA-256-PLUS"))
                    session->cap.flags |= POP3_CAP_AUTH_SCRAM_SHA_256_PLUS;
                if (token_in_string(authstring, "SCRAM-SHA-1-PLUS"))
                    session->cap.flags |= POP3_CAP_AUTH_SCRAM_SHA_1_PLUS;
                ...
                if (token_in_string(authstring, "XOAUTH2"))
                    session->cap.flags |= POP3_CAP_AUTH_XOAUTH2;
            }
        ...
        char **errmsg, char **errstr)
{
    ...
    e = pop3_send_cmd(session, errstr, "AUTH OAUTHBEARER %s", b64);
    free(oauth);
    free(b64);
    if (e != POP3_EOK)
    {
 ...
 * pop3_auth_xoauth2()
 *
 * Do POP3 authentication via AUTH XOAUTH2.
 * The POP3 server must support POP3_CAP_AUTH_XOAUTH2
 * Used error codes: POP3_EIO, POP3_EAUTHFAIL, POP3_EINVAL
 */

int pop3_auth_xoauth2(pop3_session_t *session,
        const char *user, const char *token,
        char **errmsg, char **errstr)
{
    int e;
...
{
    return (   ((session->cap.flags & POP3_CAP_AUTH_SCRAM_SHA_256_PLUS)
...
int pop3_client_supports_authmech(const char *mech)
{
    ...
    if (strcmp(mech, "USER") == 0 || strcmp(mech, "APOP") == 0
            || strcmp(mech, "OAUTHBEARER") == 0 || strcmp(mech, "XOAUTH2") == 0)
    {
        supported = 1;
    }
    else
        ...
        char **errstr)
{
    ...
    if (strcmp(auth_mech, "EXTERNAL") != 0)
    {
        ...
        /* SCRAM-*, DIGEST-MD5, CRAM-MD5, PLAIN, LOGIN, NTLM, USER, APOP,
    ...
    /* USER, APOP, OAUTHBEARER, XOAUTH2 are built-in, all other methods are provided by GNU SASL */
    ...
    {
        gsasl_done(ctx);
        e = pop3_auth_oauthbearer(session, hostname, port, user, password, errmsg, errstr);
        free(callback_password);
        return e;
    }
    ...
    {
        gsasl_done(ctx);
        e = pop3_auth_xoauth2(session, user, password, errmsg, errstr);
        free(callback_password);
        return e;
    }
    ...
    gsasl_finish(sctx);
    gsasl_done(ctx);
    /* For DIGEST-MD5, we need to send an empty answer to the last
     * response before we get OK. */
    ...
    if (strcmp(auth_mech, "") == 0)
    {
        ...
        if (mtls_is_active(&session->mtls))
        {
            ...
            else if (session->cap.flags & POP3_CAP_AUTH_CRAM_MD5)
            {
                auth_mech = "CRAM-MD5";
            }
    ...
    if (strcmp(auth_mech, "EXTERNAL") != 0)
    {
        /* CRAM-MD5, PLAIN, LOGIN, APOP, USER, OAUTHBEARER, XOAUTH2 all need a user name and
         * password */
        if (!user)
        {
            *errstr = xasprintf(_("authentication method %s needs a user name"),
