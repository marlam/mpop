...
int main(int argc, char *argv[])
{
	...
	switch(c)
	{
		...
		else if (strncmp(optarg, "simple_mbox,", 12) == 0)
		{
		    cmdline_account->delivery_method = 
			DELIVERY_METHOD_SIMPLE_MBOX;
		    free(cmdline_account->delivery_args);
		    cmdline_account->delivery_args = expand_tilde(optarg + 12);
		}
		else
		{
		    print_error(_("invalid argument %s for %s"), 
	...
	while (!list_is_empty(lp))
	{
		...
		else if (account->delivery_method == 
			DELIVERY_METHOD_SIMPLE_MBOX)
		{
		    printf("simple_mbox");
		}
