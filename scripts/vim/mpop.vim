...
syn match mpopWrongOption /\<auth \(on$\|off$\|user$\|plain$\|cram-md5$\|digest-md5$\|scram-sha-1$\|gssapi$\|external$\|login$\|ntlm$\|oauthbearer\)\@!.*$/
...
syn match mpopHost "\%(host\s*\)\@<=\%([0-9]\+\.[0-9]\+\.[0-9]\+\.[0-9]\+\)"
" Numeric values
